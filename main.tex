...
\begin{itemize}
    ...
    \item \event{DeliverCert(height, cert)}\\
    This event means that one of the \module{Consensus/TOB} modules delivered an availability certificate
    ...
    \item \event{NewCheckpoint(checkpoint)}\\
    This event means that the \module{Checkpoint} module produced a checkpoint.
    In turn, the \module{Orchestrator} initiates agreement on this new checkpoint (see \cref{sec:creating-checkpoints}).
    To this end, it creates a new instance of a \module{Consensus/TOB} module -- more precisely,
    makes the \module{Ordering} module do so -- by emitting a \event{NewModule} event to \module{Ordering},
    with the new checkpoint set as the proposal.

    \item \event{DeliverCheckpoint(checkpoint)}\\
    This event means that the \module{Consensus/TOB} module agreeing on the checkpoint reached a decision.
    ...
    by triggering their corresponding \event{GarbageCollect} events with a retention index
    that corresponds to the epoch of the delivered checkpoint.

    \item \event{EpochProgress(replica, epoch)}\\
    This event notifies the \module{Orchestrator} about a replica having advanced to a particular epoch.
    ...
    This event is triggered by the \module{Checkpoint} module whenever it receives a message from a replica.
    ...
    \item \event{RestoreFromCheckpoint(checkpoint)}\\
    When another replica considers this replica to have fallen behind (by the mechanism explained just above),
    the \module{Orchestrator} will receive the \event{RestoreFromCheckpoint} event
    ...
    by triggering a \event{ValidateCheckpoint} at the \module{Checkpoint Validator}.

    \item \event{CheckpointValidated}\\
    ...
    \item \event{PushCheckpointTimeout}\\
    The \module{Orchestrator} periodically receives this event from the \module{Timer} module.
    ...
    \item \event{NewConfig}\\
    As described in \cref{sec:configuration}, the \module{Application} module defines the system configurations.
    Those have the form of a \event{NewConfig} event received by the \module{Orchestrator},
...
the \module{Consensus/TOB} module may request the \module{Proposal Validator} to validate consensus proposals received from other replicas.
...
and notifies the \module{Consensus/TOB} about their result.

The \module{Proposal Validator} module consumes only one type of event:

\begin{itemize}
    
    \item \event{ValidateProposal(proposal)}\\
    The \module{Proposal Validator} checks the validity of the given proposal and emits a \event{ProposalValidated} event
    back to the module that emitted the \event{ValidateProposal} event (the \module{Consensus/TOB} module in our case).
...
The \module{Ordering} module is responsible for managing \module{Consensus/TOB} submodules.
...
The \module{Ordering} module consumes the following events:

\begin{itemize}

    \item \event{NewModule(moduleConfig, retentionIndex)}\\
    Upon receiving this event, the \module{Ordering} module instantiates a new \module{Consensus/TOB} submodule
    (that implements the PBFT protocol in our case).
    The created submodule is associated with the given retention index and parametrized according to \event{moduleConfig}.
    ...
    \item \event{GarbageCollect(retentionIndex)}\\
    Upon reception of this event (from the \module{Orchestrator}),
...
\subsubsection{\module{Consensus/TOB}}

The \module{Consensus/TOB} modules are dynamically created and deleted by the \module{Ordering} module
and are responsible for executing the consensus / TOB protocol.
A \module{Consensus/TOB} module is oblivious of the epoch it is part of or the other \module{Consensus/TOB} instances.
It simply receives a list of heights as part of its configuration,
and for each of these heights, it needs to agree on a value and announce it to the \module{Orchestrator} by emitting a \event{Deliver} event.

If the \module{Consensus/TOB} module is configured with a proposal, it will use that proposal as input for the consensus protocol.
...
The \module{Consensus/TOB} module consumes the following events:
\begin{itemize}

    \item \event{NewCert(cert)}\\
    In case the \module{Consensus/TOB} module has not been pre-configured with a proposal
    and requested a new certificate from the \module{Disseminator},
    the \event{NewCert} event carries a new availability certificate
    that the \module{Consensus/TOB} module uses as a consensus proposal.

    \item \event{ProposalValidated(validationResult)}\\
    This event is relevant only if the \module{Consensus/TOB} module's implementation relies on external proposal validation
    and has emitted a \event{ValidateProposal} (see \cref{sec:module-proposal-validator}) event.
    If so, the \event{ProposalValidated} event carries the result of the validation
    and triggers whatever further processing of the consensus protocol that has been waiting for it.

    \item The other events consumed by the \module{Consensus/TOB} module are specific to its implementation.
    In our case, those are events for receiving various protocol message
    (\event{Preprepare}, \event{Prepare}, \event{Commit}, \event{ViewChange}, etc.) and various PBFT-specific timeouts.
...
Similarly to the \module{Ordering} module managing instances of \module{Consensus/TOB},
...
The \module{Dissemination} module consumes the following events:

\begin{itemize}

    \item \event{NewModule(moduleConfig, retentionIndex)}\\
    Upon receiving this event, the \module{Dissemination} module instantiates a new \module{Disseminator} submodule.
    The created submodule is associated with the given retention index and parametrized according to \event{moduleConfig}.
...
Whenever a \module{Disseminator} produces a new availability certificate, it emits a \event{NewCert} event.
...
The \module{Disseminator} consumes the following events:

\begin{itemize}

    \item \event{RequestCert}\\
    This event requests a new availability certificate.
    In response, the \module{Disseminator} triggers a \event{NewCert} event containing the produced certificate.
    Note that this need not necessarily happen immediately.
    The implementation of the \module{Disseminator} may only trigger the data dissemination upon reception of a \event{CertRequest}.
    In our implementation, however, the \module{Disseminator} continuously disseminates transaction data
    and buffers the resulting certificates that it can immediately use as a response to a \event{CertRequest}

    \item \event{NewBatch(batch)}\\
    Through this event, the \module{Mempool} announces a new transaction batch ready to be disseminated.
    The \module{Mempool} emits the \event{NewBatch} event as a response
    to a \event{RequestBatch} previously emitted by the \module{Disseminator}.

    \item \event{BatchStored}\\
    The \event{BatchStored} event notifies the \module{Disseminator} that a batch has been persisted in the \module{Block Store}.
    This occurs when the \module{Disseminator} has previously asked (through a \event{StoreBatch} event)
    the \module{Block Store} to store a batch during the dissemination process.
    In our implementation, this is when the \module{Disseminator} signs the stored batch and sends a \event{Sig} message (see \cref{sec:dissemination}).

    \item \event{RequestTransactions(cert)}\\
    Upon reception of this event, the \module{Disseminator} looks up all the transactions (including their payloads) referenced by \event{cert}.
    If the referenced transactions are not present locally (in the \module{Block Store}),
    the \module{Disseminator} requests the missing transactions from replicas that have signed \event{cert}.
    The \module{Disseminator} then and emits a \event{ProvideTransactions} event containing those transactions.
    
    \item \event{BatchLookupResult}\\
    This event informs the \module{Disseminator} of the result of looking up a transaction batch in the \module{Block Store}.
...
\begin{itemize}
    \item \event{StoreBatch(batchID, batchData, retentionIndex)}
    The \module{Block Store} persistently stores \event{batchData} under \event{batchID}.
    The \event{batchID} is derived from a hash of \event{batchData}.
    The stored batch is associated with the given \event{retentionIndex} for garbage collection purposes.
    Once the batch is stored, the \module{Block Store} emits a \event{BatchStored} event.

    \item \event{LookUpBatch(batchID)}
    The \module{Block Store} looks up the given \event{batchID} in its local database and emits a \event{BatchLookupResult} event
    containing either the corresponding batch data or a flag that the batch was not found.
    
    \item \event{GarbageCollect(retentionIndex)}
    This event makes the \module{Block Store} module delete all batches associated with a retention index smaller than \event{retentionIndex}.
    The \module{Block Store} receives this event from the \module{Orchestrator} each time a new checkpoint is created.
...
our implementation includes a \module{Mempool} module that accepts \event{NewTransactions} events and stores the contained transactions in memory.
Upon receiving a \event{RequestBatch} event, the \module{Mempool} responds with a \event{NewBatch} event containing the stored transactions.
...
When the \module{Mempool} emits a transaction batch, the batch still needs to be proposed, ordered, and executed.
...
At the same time, transactions that \emph{are going to be} executed (and checkpointed) should not be included in new batches emitted by the \module{Mempool}.
However, the \module{Mempool} cannot know which of the already emitted transactions are going to make it into a checkpoint and which are not.
...
Instead, we make the \module{Mempool} aware of Trantor epochs and allow it to emit each transaction only once per epoch.
...
The \module{Mempool} consumes the following events:

\begin{itemize}

    \item \event{NewTransactions(transactions)}\\
    The \module{Mempool} stores all the provided \event{transactions} that are within their respective client's watermark windows.

    \item \event{RequestBatch}
    The \module{Mempool}, according to its parametrization, eventually emits a \event{NewBatch} event.

    \item \event{BatchTimeout}
    When waiting for a batch to fill, the \module{Mempool} sets up a timer
    (by emitting a \event{TimerDelay} event to the \module{Timer} module)
    to receive this event after \param{BatchTimeout}.
    Upon reception of this event, the \module{Mempool} emits a \event{NewBatch} event with the corresponding batch, unless it has already done so.

    \item \event{NewEpoch(executedTXs)}
    The \module{Mempool} receives this event at the start of every epoch to
    \begin{itemize}
        \item update its view of the client watermark windows according to \event{executedTXs},
        \item delete all \event{executedTXs}, and
...
On reception of an event, the \module{Application} module simply calls the corresponding function of the execution component
and emits an event with that function's return value (if any).

The \module{Application} module consumes the following events:

\begin{itemize}

    \item \event{OrderedBatch(batch)}
    The \module{Application} module calls the \code{ApplyTXs(transactions)} function of the application.

    \item \event{SnapshotRequest}
    The \module{Application} module calls the \code{Snapshot()} function of the application
    and emits a \event{Snapshot} event containing its return value.
    This \event{Snapshot} event goes directly to the \module{Checkpoint} module responsible for creating the checkpoint the snapshot is part of.

    \item \event{RestoreState(checkpoint)}
    ...
    and emits a \event{NewConfig} event containing the returned configuration.
...
In general, it is mostly the \module{Orchestrator} that assembles a sequence of events to be consumed by the \module{Application}.
...
It is only the \event{OrderedBatch} events that are missing in this sequence -- the \module{Orchestrator} emits \event{OrderedCert}s in their place.

The \module{Block Fetcher}'s main task is to replace the \event{OrderedCert} events in this sequence by the corresponding \event{OrderedBatch} events
and deliver the sequence to the \module{Application}.
When constructing the \event{OrderedBatch} events, the \module{Block Fetcher} also removes duplicate transactions as described in \cref{sec:tx-deduplication}.

Replacing an \event{OrderedCert} by an \event{OrderedBatch} involves communication with the corresponding \module{Disseminator}
which, in turn, might even need to communicate with other replicas over the network.
In the meantime, the \module{Block Fetcher} might receive more events from the \module{Orchestrator}.
The \module{Block Fetcher} can buffer such events and delay their processing until it has finished fetching the batch.
This is required if the content of the batch can change the \module{Block Fetcher}'s state (e.g., the information about delivered transactions)
on which the processing of subsequent events depends.

The \module{Block Fetcher} keeps track of the current epoch by observing the \event{NewEpoch} events emitted by the \module{Orchestrator}.
It uses the epoch information to always request the transaction batches from the correct \module{Disseminator} instance.

The \module{Block Fetcher} consumes the following events:

\begin{itemize}

    \item \event{OrderedCert(cert)}\\
    The \module{Block Fetcher} requests the transaction batch from the current epoch's \module{Disseminator} instance
    by emitting a \event{RequestTransactions} event.

    \item \event{ProvideTransactions(transactions)}\\
    This event is the \module{Disseminator}'s response to the previously emitted \event{RequestTransactions} event.
    Upon receiving it, the \module{Block Fetcher} assembles a new batch from only those \event{transactions}
    that have not yet been delivered to the \module{Application} and creates a new \event{OrderedBatch} event.
    The \module{Block Fetcher} then either directly emits the new \event{OrderedBatch} event
    or, in case a previous event is still waiting to be emitted, enqueues it for later in-order emission.

    \item \event{SnapshotRequest}
    This event is emitted by the \module{Orchestrator} during the creation of a checkpoint.
    The \module{Block Fetcher} relays this event to the \module{Application}.
    Moreover, since the \module{Block Fetcher} also contains state than must be checkpointed (the client watermark information),
    it serializes the relevant part of its state and emits a \event{ClientProgress} event directly to the corresponding \module{Checkpoint} module.

    \item \event{RestoreState(checkpoint)}
    The \module{Block Fetcher} relays this event to the \module{Application} and re-initializes the relevant parts of its own state
    (epoch number and client watermark information).

    \item \event{NewEpoch}
    The \module{Block Fetcher} updates its local view of the epoch number and relays the event to the \module{Application}.
    It also emits a \event{NewEpoch} event to the \module{Mempool} with information about the transactions executed so far (see \cref{sec:module-mempool}).
...
The \module{Checkpointing} module consumes the following events:

\begin{itemize}

    \item \event{NewModule(moduleConfig, retentionIndex)}\\
    Upon receiving this event, the \module{Checkpointing} module instantiates a new \module{Checkpoint} submodule.
    The created submodule is associated with the given retention index and parametrized according to \event{moduleConfig}.
...
arrives later in, respectively a \event{Snapshot} event and a \event{ClientProgress} event.
...
The \module{Checkpoint} module consumes the following events:

\begin{itemize}
    \item \event{Snapshot(appSnapshot)}\\
    Delivers the snapshot of the application state to be checkpointed.
    If the \event{ClientProgress} has already been received, the module starts the checkpointing protocol.
    
    \item \event{ClientProgress(clWatermarkData)}\\
    Delivers the most up-to-date client watermark infromation.
    If the \event{Snapshot} has already been received, the module starts the checkpointing protocol.

    \item The other events consumed by the \module{Checkpoint} module are specific to its implementation.
...
The \module{Checkpoint Validator} module consumes the following event:

\begin{itemize}

    \item \event{ValidateCheckpoint(checkpoint)}\\
    Validates the given checkpoints and emits a \event{CheckpointValidated} event with the validation result.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \event{SendMessage(message, destinations)}
    \end{itemize}
    
    \item \module{Timer}
    \begin{itemize}
        \item \event{DelayEvent(event, delay)}
        \item \event{RepeatEvent(event, period, retentionIndex)}
        \item \event{GarbageCollect(retentionIndex)}
    \end{itemize}
    ...
    \item \module{Crypto}
    \begin{itemize}
        \item \event{Sign(data)}
        \item \event{VerifySig(data, signer)}
    \end{itemize}
...
Our mempool implementation is parametrized with \param{MaxTransactionsInBatch} = 2048 (chosen empirically after exploring a few others),
\param{MaxPayloadInBatch} = 16MB (a very large value that effectively disables this limit), and a varying \param{BatchTimeout}.
