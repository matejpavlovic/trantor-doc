...
In addition to the transaction log data structure, the \module{Orchestrator} maintains the configuration
(including replica membership information) for the current and \param{ConfigOffset} next epochs,
as well as the most recent checkpoint.
It keeps track of the progress of other replicas in order to decide when they need to be sent a checkpoint to catch up.
