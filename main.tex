...
\subsection{\module{Consensus/TOB}}
...
It only receives a list of heights as part of its configuration,
for each of which it needs to deliver an agreed-upon value to the \module{Orchestrator} by emitting a \event{Deliver} event.
...
Otherwise, it will request an availability certificate from a \module{Certificate Creator} module it is configured to use.

The \module{Consensus/TOB} module consumes the following events:
\begin{itemize}

    \item \event{NewCert(cert)}\\
    In case the \module{Consensus/TOB} module has not been pre-configured with a proposal for some height
    and requested a new certificate from the \module{Certificate Creator},
