...
\item Information about processed transactions (needed for deduplication, see Section \cref{sec:tx-deduplication})
\end{compactitem}
...
\begin{itemize}
    ...
    \item \textit{MaxTransactionsInBatch}: An emitted \event{NewBatch} event never contains more than \textit{MaxTransactionsInBatch} transactions.
    ...
    \item \textit{BatchTimeout}: To enable effective batching of transactions, the mempool does not respond to a \event{BatchRequest} immediately
    ...
    After \textit{BatchTimeout} elapses, the mempool emits a \event{NewBatch}, even if it is not full (or is even completely empty).
\end{itemize}
...
% \paragraph{\textit{SegmentLength} parameter.} Being heavily inspired by ISS \cite{iss},
...
% The \textit{SegmentLength} is the number of batches each replica needs to propose (and have the system agree upon) in each epoch.
% Thus, for example, for a \textit{SegmentLength} of 2 and 32 replicas, the epoch length is 64
...
% For all experiments except for one, we use a \textit{SegmentLength} of 2.
